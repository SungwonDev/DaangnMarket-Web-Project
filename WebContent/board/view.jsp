...
<c:set var="fListidx" value="${rboardDAO.selectListidx()}"/>
...
<!--  script/ -->
	...
	%>
		...
		<c:set var="selectList" value="${rboardDAO.selectList(jstlnumindex)}"/>
		            	...
		            	<div class="swiper-container">
						    <div class="swiper-wrapper">
						     ...
						     <c:choose>
						      ...
						      <c:choose>
						     	 <c:when test="${empty item.file2}">
						     	 	
							      </c:when>
							      <c:when test="${not empty item.file2}">
										<div class="swiper-slide" id="sli2">
								      		<img alt="${item.file2}" src="/Corrot/upload/${item.file2}?q=82&amp;s=300x300&amp;t=crop">
								      	</div>
								 </c:when>
						       </c:choose>
						       
						      <c:choose>
						     	 <c:when test="${empty item.file3}">
						     	 	
							      </c:when>
							      <c:when test="${not empty item.file3}">
										<div class="swiper-slide" id="sli3">
								      		<img alt="${item.file3}" src="/Corrot/upload/${item.file3}?q=82&amp;s=300x300&amp;t=crop">
								      	</div>
								 </c:when>
						       </c:choose>
						       
						      <c:choose>
						     	 <c:when test="${empty item.file4}">
						     	 									      
							      </c:when>
							     <c:when test="${not empty item.file4}">
										<div class="swiper-slide" id="sli4">
								      		<img alt="${item.file4}" src="/Corrot/upload/${item.file4}?q=82&amp;s=300x300&amp;t=crop">
								      		<h2>바이</h2>
								      	</div>
								 </c:when>
						       </c:choose>
						      ...
						      <c:choose>
						     	 <c:when test="${empty item.file5}"> 
						     		 
								      
								      </c:when>
								      <c:when test="${not empty item.file5}">
										<div class="swiper-slide" id="sli5">
								      	<img alt="${item.file5} " src="/Corrot/upload/${item.file5}?q=82&amp;s=300x300&amp;t=crop">
								      	</div>
								 	</c:when>
						       </c:choose>	
